...
const carouselDieukhung = document.querySelector(".GiatriDieuKhung");
firstImg = carousel.querySelectorAll("img")[0];
arrowIcon = document.querySelectorAll(".DieuKhung i");

let isDragStartDieukhung = false;
let frittImgWidDieukhung = firstImg.clientWidth;

arrowIcon.forEach(icon => {
  icon.addEventListener("click", () => {
    console.log(icon);
    const speedFactor = 4; // Hệ số tốc độ
    carouselDieukhung.scrollLeft += icon.id === "leftDieuKhung" ? -frittImgWidDieukhung * speedFactor : frittImgWidDieukhung * speedFactor;
  });
});
